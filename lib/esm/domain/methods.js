...
export const handleSetDomainDefaults = function(data){
	...
	const self = this 
	const domains =  self?.domainCandidates ? self.domainCandidates :  data.custom.wepackMiddlewares
	...
	domains.forEach(dumain => {
		...
		if(dumain.name === "webpackDevMiddleware") return data.app.use(dumain.use(data.custom.compiler,{
			
		}))
		if( dumain.name === "webpackHotMiddleware" ) return data.app.use(dumain.use(data.custom.compiler))
...
export const hookIntoWebpackCompilation = async function (compiler) {
	...
	self.infoSync(stats.toJson({all:false, errors:true, warnings: true}));
