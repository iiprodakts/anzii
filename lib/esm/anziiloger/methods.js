...
export const handleLogRequest = function(data){
	...
	if(pao.PROMPT.indexOf('cli') >= 0) return
	...
	if(self.logger){
		...
		switch(data.type){
			...
			break
			case 'adLog': self.aLog(data)
			break
			case 'wiLog': self.iLog(data)
			break
...
export const info = async function(log){
	...
	if(contains(log,'sync')){ 
		...
		if(self.debugas.hasOwnProperty(log.source.toLowerCase())){
			...
			!self.debugas[log.source.toLowerCase()].enabled && proccess.env.NODE_ENV.toLowerCase() === 'production' ? self.debugas[log.source.toLowerCase()].enabled = true : '' 
