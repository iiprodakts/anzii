    ...
    // self.infoSync(conn.query)
     ...
     conn.getConnection(async function(err,connection){
        ...
        for(let q=0; q < query.length; q++){
          ...
          if(currentResult.FIND_ITERATE_ERROR){
  ...
  // let set = sets.map((f,i)=>{})
	...
	return new Promise(function(resolve,reject){
      ...
      self.updateOne({
        ...
        connector,
        update: update,
        query:{set:[...Object.entries(update.fields).map((f,i)=> {return {[f[0]]: f[1]}}
          )],conditions:[update.condition],table: update.name},
        outComehandler: (e=null,r=null,data=null)=>{
                ...
                if(e){
                  ...
                  if(r.changedRows && r.changedRows > 0){
