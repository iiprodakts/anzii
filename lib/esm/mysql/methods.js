...
export const findIterateItemPromise = function (conn, sql) {
    ...
    return new Promise((resolve, reject) => {
        conn.query(sql, function (e, r, f) {
            ...
            self.infoSync(r);
            if (e)
                return reject({ FIND_ITERATE_ERROR: true, e: e });
            return resolve(r);
            // if(pao.pa_isArray(r) && r.length > 0 && query.length !== 1 ){ 
            //     if(find.alias){
            //       if(result instanceof Array){
...
export const PROCEDURE = async function (collections, conn, handler = null, connector) {
    const self = this;
    const pao = self.pao;
    ...
    conn.getConnection(async (err, connection) => {
        ...
        for (let c = 0; c < collections.length; c++) {
            ...
            if (!pao.pa_contains(i, 'fields') && (i.action && i.action !== 'delete')) {
                ...
                if (pao.pa_contains(i.fields, 'tables')) {
                    ...
                    if (i.fields.own instanceof Array) {
                        let dicts = i.fields.own.map((doc, doci) => {
                            return {
                                ...doc
                            };
                        });
            ...
            switch (action) {
                case "insert":
                    {
                        await self.insert(i, connection, connector)
                            .then((insert) => {
                            collectionsIds.push(insert);
                        })
                            .catch((failedInsert) => {
                ...
                case "update":
                    {
                        await self.procedureUpdate(i, connection, connector)
                            .then((update) => {
                ...
                case "delete":
                    {
                        await self.procedureDelete(i, connection, connector)
                            .then((deleted) => {
                ...
                case "select":
                    {
                        await self.procedureSelect(i, connection, connector)
                            .then((selected) => {
                            self.infoSync('THE DELETED');
                    ...
                    break;
                default: throw new Error("No valid table procedure action has been defined");
            }
            ...
            if (c === collections.length - 1) {
                ...
                if (collectionsIds.length > 0) {
                    ...
                    if (collectionsIds[0].isDelete) {
                        savedData = collectionsIds.map((sd, i) => {
                            return {
...
export const procedureUpdate = function (update, conn, connector) {
    ...
    return new Promise(function (resolve, reject) {
        self.updateOne({
            ...
            connector,
            update: update,
            query: { set: [...Object.entries(update.fields).map((f, i) => { return { [f[0]]: f[1] }; })], conditions: [update.condition], table: update.name },
            outComehandler: (e = null, r = null, data = null) => {
                ...
                if (e) {
                    ...
                    if ((r.affectedRows && r.affectedRows > 0) || (r.changedRows && r.changedRows > 0)) {
                        ...
                        update.fields = data.updateKey && data.updateKeyID ? { ...data.fields, [data.updateKeyID]: data.updateKey } : { ...data.fields };
                        update.collection = data.name;
                        update.collectionAlt = data.altName ? data.altName : '';
    ...
    switch (fieldList[0]) {
        ...
        case 'field':
            if (fieldList.length === 3)
                fieldstatement = `${fieldList[1]} AS ${self.options(fieldList[2], 'as')}`;
            break;
        default:
            fieldstatement = '';
    }
