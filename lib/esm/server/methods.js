...
export const handleDomainResources = function (data = null) {
    ...
    self.emit({ type: 'set-domain-defaults', data: { app: self.http, xpress: self.xpress, custom: data.payload } });
};
...
export const handleWriteServerRequestResponse = async function (data) {
    ...
    if (data.method === 'stream') {
        ...
        data.R_ID
            ...
            : self.infoSync(`SERVER IS ABOUT TO SEND RESPONSE BACK TO CLIENT WITH RESPONSE`);
        ...
        if (data.data.accepts) {
            switch (data.data.accepts) {
                case 'json':
                    ...
                    break;
                case 'html':
                    self.getHtml(data.res, { view: 'main/404', title: "Page could not be found" })
                        .then((html) => {
...
export const streamResponse = function (data) {
    ...
    if (withAttachment)
        data.res['withAttachment'] = { ...withAttachment };
        ...
        res.render(view.view, viewda, (err, html) => {
            if (err) {
                ...
                res.render('main/500', { title: 'Server error' }, (err, html) => {
                    if (err)
                        return resolve({ html: '<hl>The page could not be found</h1>', success: false });
