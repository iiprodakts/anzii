...
export const connectToClient = function(client){
	...
	self.adLog(`System is about to connect to client: ${client.name}`) 
...
export const connect = function(client){
	...
	self.info(`System is connecting to client: ${client.name}`) 
		...
		let res = sclient.driver[sclient.connectMethod](opts,(err,res)=>{
			  ...
			  if(err){
				...
				self.pao.pa_wiLog(err.stack)
				  ...
				  self.info("System has successfully connected to client") 
				  self.info(`Client ready to serve queries`)
