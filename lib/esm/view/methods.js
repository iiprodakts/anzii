...
export const handleConfigView = function(data){
	...
	self.logSync('views data from config')
	self.logSync(data)
	...
	routes.forEach(route => {
		...
		if(route.view){
			...
			if(route.viewty){
				...
				self.logSync('THE VIEW HANDLER')
				self.logSync(route.viewso.indexOf('/'))
				// self.logSync(route.viewso.indexOf('/'))
				self.logSync(handlers[handlerView])
				self.logSync(handlerView)
...
export const handleViewTask = async function(data){
	...
	if(checkViewStatus){
		...
		if(checkViewStatus.type === 'template'){
			...
			self.logSync('about to render template view')
...
export const isTemplateView = function(views,view){
	...
	self.logSync('ISVALID AS FILTERED')
	self.logSync(isValidView)
