...
export function p_wiLog(message) {
    if (!process.env.SHOW_WILD_LOGS)
        return;
    if (process.env.SHOW_WILD_LOGS.trim().toLowerCase() === 'false')
        return;
    console.log(message);
}
...
// export const find_in = moduleExports.find_in;
    ...
    export const p_loadFile = function (filepath) {
        ...
        return new Promise(async (resolve,reject)=>{
            ...
            if(!p_isExistingDir(filepath)) return reject(new Error({code:"FILE_PATH_ERROR",message:"File path does not exist"}))
            ...
            import(filepath).then((moduleFound)=>{
    ...
    export const p_loadFileSync = function (filepath) {
        ...
        const redIS = require(filepath)
        console.log("THE REDIS", redIS)
