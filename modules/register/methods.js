...
export const init = function(){
  ...
  export const handleRegisterTask = async function(data){
	...
	switch(user.action){
		 ...
		 case 'registerUser':{
			   ...
			   self.registerUser(user.payload)
			  .then(async (userStatus)=>{
				  await self.log('THE Register Status')
				  await self.log(userStatus)
				  
				  if(userStatus) return self.callback(null,{message: 'exists'})
				  
				   self.addUser(user.register)
				   .then((addedUser)=>{
					   
						self.createJwt(addedUser)
						.then((jwt)=>{
							
							 self.sendEmail(addedUser)
							 .then((mailSent)=>{
  ...
  export const registerUser = function(pay){
	  ...
	  let user = pay
		  ...
		  return new Promise((resolve,reject)=>{
			  ...
			  self[user.strategy](user.register)
			  .then((registerStatus)=>{
  ...
  export const anzii = function(user){
	  ...
	  return new Promise((resolve,reject)=>{
			 ...
			 if(!pao.pa_isValidEmail(user.email) && !pao.pa_isValidPassword(user.password)) return reject(new Error('Invalid register input'))
			  ...
			  self.isUserExist(resolve,reject,user) 
			  .then((existStatus)=>{
  ...
  export const addUser = function(user){
	...
	return new Promise((resolve,reject)=>{
		 ...
		 self.emit({type:"hash-payload",data: {payload: user.password,callback:hash.bind(self,resolve,reject)}})
  ...
  export const setTokenHeader = function(reslve,reject,e=null,token=null){
  ...
  const hash = async function(resolve,reject,e=null,h=null){
  
	  const self = this 
	  if(e){
  
		  console.log(e)
		  reject({message: 'registration failed due to server error:hash'})
		  
	  }else{
  
		  console.log('hashed')
		  console.log(h)
		  let user = self.tmpd.user
		  let password = h 
  ...
  const handleSentEmail = function(sendStatus){
	  
  }
  
  const getJwt = async function(resolve,reject,e=null,h=null){
  ...
  export const createJwt = function(user){
	...
	const self = this
	  ...
	  return new Promise((resolve,reject)=>{
		  ...
		  data:{payload: user,callback: self.setTokenHeader.bind(self,resolve,reject)}})
  ...
  export const sendEmail = function(user){
	  
	const self = this
	  
	  return new Promise((resolve,reject)=>{
		  
			  self.emit({type:'send-mail',
			  data:{payload: user,
  ...
  export const hookFunkToThingy = function(hooky,hook,args){
