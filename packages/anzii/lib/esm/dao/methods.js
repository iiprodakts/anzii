...
export const handleDataHiveRequest = function (data) {
    ...
    self.adLog(data);
    if (!self.DBS)
        throw new Error('No database vendor configured');
    if (!pao.pa_contains(self.DBS, data.model.vendor)) {
        ...
        self.emit({
            ...
            data: { conn: self.DBS[model.vendor], connector: self.connectors[model.vendor], table: model.table, opi: model.operation, query: data.document, outComehandler: data.handler }
        });
    }
};
